/*
  Original Copyright (C) 2012 Joseph J. Pfeiffer, Jr., Ph.D. <pfeiffer@cs.nmsu.edu>
...
void log_fuse_context(struct fuse_context *context)
{
    ...
    log_struct(((struct _tfs_state *)context->private_data), logfile, %08x, );
    log_struct(((struct _tfs_state *)context->private_data), devpath, %s, );
...
void log_conn(struct fuse_conn_info *conn)
{
    ...
    /** Maximum size of the write bufer */
    // unsigned max_write;
